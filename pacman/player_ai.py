...
def _bfs_multi_source_distance_field(level: Level, starts: set[Cell]) -> dict[Cell, int]:
    dist: dict[Cell, int] = {}
    queue: deque[Cell] = deque()
    for s in starts:
        if _is_walkable_cell(level, s[0], s[1]):
            dist[s] = 0
            queue.append(s)
...
class BerkeleyExpectimaxAgent:
    """Bản port cực kỳ tối ưu của thuật toán Expectimax từ multiAgents.py.
    Đã được nâng cấp để chạy mượt và cực thông minh trên Pygame bằng cách:
    1. Dùng khoảng cách đường thực tế (BFS) thay vì đường chim bay (Manhattan).
    2. Cắt tỉa (chỉ tính 2 con ma gần nhất) để có thể đẩy depth=2.
    3. Cải thiện logic né ma trong hàm đánh giá có khoảng cách gradient.
    """
    ...
    def __init__(self, depth: int = 2) -> None:
        ...
        self._bfs_dist: dict[tuple[int, int], int] = {}
        self._food_dist: dict[tuple[int, int], int] = {}
    ...
    def get_action(self, player: "Player", ghosts: list["Ghost"], level: Level) -> int:
        ...
        if (
            self._cached_action is not None
            and self._frame_counter % 6 != 0
            and player.turns_allowed[self._cached_action]
        ):
            return self._cached_action

        # 1. Tính toán khoảng cách BFS từ Pacman tới mọi ô (dùng để định hướng chính xác)
        player_cell = (
            (player.y_pos + PLAYER_CENTER_OFFSET_Y) // CELL_H,
            (player.x_pos + PLAYER_CENTER_OFFSET_X) // CELL_W,
        )
        self._bfs_dist = _bfs_distance_field(level, player_cell)
        
        # 2. CHỈ đưa 2 con ma gần nhất vào thuật toán để giảm branching factor, cho phép chạy depth=2
        ghost_distances = []
        for i, g in enumerate(ghosts):
            ...
            # Ưu tiên dùng BFS distance, nếu lỗi thì dùng manhattan
            dist = self._bfs_dist.get(gc, _manhattan(player_cell, gc))
            ghost_distances.append((dist, i))
            
        ghost_distances.sort(key=lambda x: x[0])
        active_ghost_indices = [x[1] for x in ghost_distances[:2]]
        active_ghosts = [ghosts[i] for i in active_ghost_indices]

        state = self._build_state(player, active_ghosts, level)
        
        # Compute exact BFS distance field from ALL food dots (Multi-source BFS)
        # This provides a perfect, smooth gradient to the closest remaining food!
        self._food_dist = _bfs_multi_source_distance_field(level, state.dots | state.power_dots)
        self._current_direction = player.direction
        
        action, value = self.value(state, level, 0)
    ...
    def _build_state(self, player: "Player", ghosts: list["Ghost"], level: Level) -> GameState:
        return ExpectimaxAgent._build_state(player, ghosts, level)

    def value(self, state: GameState, level: Level, depth: int) -> tuple[int | None, float]:
        num_agents = 1 + len(state.ghosts)
        
        if depth == self.depth * num_agents or self._is_win(state) or self._is_lose(state):
            return None, self.betterEvaluationFunction(state, level)
        
        if depth % num_agents != 0:
            return self.expectvalue(state, level, depth)
        else:
            return self.maxvalue(state, level, depth)

    def expectvalue(self, state: GameState, level: Level, depth: int) -> tuple[int | None, float]:
        num_agents = 1 + len(state.ghosts)
        agent_index = depth % num_agents
        ghost_idx = agent_index - 1
        
        legal_actions = self._legal_ghost_actions(state, ghost_idx, level)
        if len(legal_actions) == 0:
            return None, self.betterEvaluationFunction(state, level)
        
        weight_probability = 1.0 / len(legal_actions)
        expect_val = 0.0
        
        for action in legal_actions:
            successor = ExpectimaxAgent._apply_ghost(state, ghost_idx, action)
            _, val = self.value(successor, level, depth + 1)
            expect_val += val * weight_probability
            
        return None, expect_val

    def maxvalue(self, state: GameState, level: Level, depth: int) -> tuple[int | None, float]:
        legal_actions = self._legal_pacman_actions(state, level)
        if len(legal_actions) == 0:
            return None, self.betterEvaluationFunction(state, level)
        ...
        for action in legal_actions:
            successor = ExpectimaxAgent._apply_pacman(state, action)
            _, val = self.value(successor, level, depth + 1)
            
            # Ưu tiên giữ hướng đi cũ (chống lắc)
            if depth == 0:
                if action == self._current_direction:
                    val += 0.0001
                elif action == REVERSE.get(self._current_direction):
                    val -= 0.0001
    ...
    def betterEvaluationFunction(self, state: GameState, level: Level) -> float:
        pacman_position = state.pacman
        ghost_near = 0
        scared_timer = state.powerup_timer
        
        # Đánh giá độ nguy hiểm của ma bằng Manhattan (để tính nhanh trong các node lá)
        for ghost_pos in state.ghosts:
            ghost_distance = _manhattan(ghost_pos, pacman_position)
            if scared_timer == 0:
                # Gradient né ma: càng gần càng trừ nặng, giúp Pacman biết sợ từ xa!
                if ghost_distance == 0:
                    ghost_near -= 10000  # Chết
                ...
                # Săn ma khi có powerup
                if ghost_distance > 0:
                    ghost_near += 300 / ghost_distance
                else:
                    ghost_near += 300

        # Lấy khoảng cách ngắn nhất đến thức ăn CỰC KỲ CHÍNH XÁC nhờ Multi-source BFS
        min_food_dist = self._food_dist.get(pacman_position, UNREACHABLE_DISTANCE)
                    
        # Score âm dựa trên khoảng cách tới thức ăn gần nhất
        food_score = -5 * min_food_dist if min_food_dist != UNREACHABLE_DISTANCE else 0
        num_capsules = len(state.power_dots)

        # Trọng số:
        # Score (+10 mỗi hạt)
        # Bị trừ điểm nếu còn nhiều hạt (-5 * len)
        # Bị trừ điểm nếu còn nhiều power (-200)
        return state.score * 10 + ghost_near - 5 * len(state.dots) - 200 * num_capsules + food_score

    def _is_win(self, state: GameState) -> bool:
        return not state.dots and not state.power_dots

    def _is_lose(self, state: GameState) -> bool:
        if state.powerup_timer <= 0:
            for ghost in state.ghosts:
                if ghost == state.pacman:
                    return True
    ...
    def _legal_ghost_actions(self, state: GameState, ghost_idx: int, level: Level) -> list[int]:
        gr, gc = state.ghosts[ghost_idx]
        ghost_dir = state.ghost_dirs[ghost_idx]
        forbidden = REVERSE.get(ghost_dir)
        actions: list[int] = []
