...
def build_wall_surface(level: Level, color: str) -> pygame.Surface:
    ...
    for row_idx, row in enumerate(level):
        for col_idx, tile in enumerate(row):
            ...
            x = col_idx * CELL_W
            y = row_idx * CELL_H
            cx = x + ARC_HALF_RATIO * CELL_W
            cy = y + ARC_HALF_RATIO * CELL_H
            if tile == VERTICAL_WALL_TILE:
                pygame.draw.line(surf, color, (cx, y), (cx, y + CELL_H), WALL_WIDTH)
            elif tile == HORIZONTAL_WALL_TILE:
                pygame.draw.line(surf, color, (x, cy), (x + CELL_W, cy), WALL_WIDTH)
            elif tile == TOP_RIGHT_WALL_TILE:
                pygame.draw.line(surf, color, (x, cy), (cx, cy), WALL_WIDTH)
                pygame.draw.line(surf, color, (cx, cy), (cx, y + CELL_H), WALL_WIDTH)
            elif tile == TOP_LEFT_WALL_TILE:
                pygame.draw.line(surf, color, (cx, cy), (x + CELL_W, cy), WALL_WIDTH)
                pygame.draw.line(surf, color, (cx, cy), (cx, y + CELL_H), WALL_WIDTH)
            elif tile == BOTTOM_LEFT_WALL_TILE:
                pygame.draw.line(surf, color, (cx, y), (cx, cy), WALL_WIDTH)
                pygame.draw.line(surf, color, (cx, cy), (x + CELL_W, cy), WALL_WIDTH)
            elif tile == BOTTOM_RIGHT_WALL_TILE:
                pygame.draw.line(surf, color, (cx, y), (cx, cy), WALL_WIDTH)
                pygame.draw.line(surf, color, (x, cy), (cx, cy), WALL_WIDTH)
            elif tile == GATE_TILE:
